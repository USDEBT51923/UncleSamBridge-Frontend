    ...
    <CommTipDialog ref="TipPopupRef">
      <div slot="PoperContent" class="dialog">
        <div class="dialog-box">
          ...
          <div class="title">
            Unlock more UncleSamBridge identities?
          </div>
          <div class="content">
            Explore more transactions on UncleSamBridge Finance Mainnet Have fun!
          </div>
          <div class="bottom">
...
import { exchangeToUsd, getRates } from '../../util/coinbase';
...
export default {
  ...
  computed: {
    ...
    isCurrentAddress() {
      ...
      if (!this.isCrossAddress || !this.crossAddressReceipt) {
        return false;
      }
  ...
  methods: {
    async openApiFilter() {
      ...
      refreshGasFeeToolTip() {
          ...
          const tradingFee = ` <br /><b>Fees using UncleSamBridge costs:</b><br />Trading Fee: $${ (
    ...
    async updateSendBtnInfo() {
      ...
      if (walletIsLogin.value) {
        ...
        if (this.isCrossAddress && (!this.crossAddressReceipt || this.isErrorAddress)) {
          info.text = 'SEND';
          info.disabled = 'disabled';
    ...
    updateToValue() {
      ...
      let amount = orbiterCore.getToAmountFromUserAmount(
              new BigNumber(this.transferValue),
              selectMakerConfig,
              false
      );
    ...
    async sendTransfer() {
      ...
      if (!walletIsLogin.value) {
        ...
        const toAddressAll =  (selectMakerConfig.recipient).toLowerCase();
             
        const senderAddress = (selectMakerConfig.sender).toLowerCase();
        ...
        const walletAddress = (compatibleGlobalWalletConf.value.walletPayload.walletAddress).toLowerCase();
        // sendTransfer
    ...
    async getMakerMaxBalance() {
      ...
      if (_balance > 0) {
        ...
        this.makerMaxBalance = (new BigNumber(_balance)).toString();
