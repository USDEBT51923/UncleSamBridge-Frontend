...
import { Coin_ABI, ZERO_ABI_ETH, NATIVE_OFT_ABI, ZERO_ABI_BASE } from './constants/contract/contract.js'
...
export default {
  ...
  GetZeroContractAddress(chainId) {
    return env.ZeroContractAddress[chainId]
  },
  GetZeroNativeContractAddress(chainId) {
    return env.ZeroNativeContractAddress[chainId]
  },
  GetZeroNativeOFTContractAddress(chainId) {
    return env.ZeroNativeOFTContractAddress[chainId]
  },
  GetZeroERC20TokenAddress(chainId) {
    return env.ZeroERC20TokenAddress[chainId]
  },
  GetZeroChainId(chainId) {
    return env.ZeroChainIds[chainId]
  },
  toHex(num) {
  ...
  getWeb3TokenBalance(chainId, userAddress, tokenAddress) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (!result) {
        reject(`Request Web3 getWeb3TokenBalance TokenBalance RPC error${chainId}`)
      }
    })
  },
  getWeb3Fee(chainId, userAddress, tokenAddress) {
    const rpcList = this.getRpcList(chainId)
    return new Promise(async (resolve, reject) => {
      let result
      if (rpcList && rpcList.length > 0) {
      ...
      if (!result) {
        reject(`Request Web3 getWeb3Fee TokenBalance RPC error${chainId}`)
      }
    })
  },
  getZeroFee(chainId, tochainId, userAddress, amount, adapterParams) {
    const rpcList = this.getRpcList(chainId)
    return new Promise(async (resolve, reject) => {
      let result
      if (rpcList && rpcList.length > 0) {
        for (const url of rpcList) {
          try {
            const web3 = new Web3(url)
            // result = await web3.eth[method](...args)
            let tokenContract = new web3.eth.Contract(ZERO_ABI_ETH, this.GetZeroContractAddress(chainId))
            if (chainId === 21) {
              tokenContract = new web3.eth.Contract(ZERO_ABI_BASE, this.GetZeroContractAddress(chainId))
            }
            ...
            const result = await tokenContract.methods
              .estimateSendFee(this.GetZeroChainId(tochainId), userAddress, amount, false, adapterParams)
              .call()
      ...
      if (!result) {
        reject(`Request Web3 getZeroFee TokenBalance RPC error${chainId}`)
      }
    })
  },
  getZeroNativeFee(chainId, toChainID, userAddress, amount, adapterParams) {
    const rpcList = this.getRpcList(chainId)
    return new Promise(async (resolve, reject) => {
      let result
      if (rpcList && rpcList.length > 0) {
        for (const url of rpcList) {
          try {
            ...
            const tokenContract = new web3.eth.Contract(NATIVE_OFT_ABI, this.GetZeroNativeOFTContractAddress(chainId))
            ...
            const result = await tokenContract.methods
              .estimateSendFee(this.GetZeroChainId(toChainID), userAddress, amount, false, "0x")
              .call()
            this.setStableRpc(chainId, url, 'success')
            resolve(result.nativeFee)
            break
          } catch (error) {
            this.log(
              'Request Web3 token Balance rpc error:',
